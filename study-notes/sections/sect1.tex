...
\begin{intuition}
Skewness can be written as \(\expv{(X-\mu)/\sigma^{3}}\). Since the term inside
is raised to power 3, long right (left) tail contributes very positively
(negatively) to skewness value.
\end{intuition}
...
above (below) 3, it is ``flatter'' (``less flat'') than normal distribution, in
the sense that more mass of distribution is located away from mean relative to
a normal distribution, keeping standard deviation constant.
\begin{center}
\begin{tikzpicture}
...
\[
X=\int_{0}^{X}1\dd{t}=\int_{0}^{\infty}\indicset{t<X}\dd{t}\footnote{\(\indicset{\cdot}\) denotes the indicator function.}
\]
Thus,
\[
...
(nonnegative) \(X\): area under the graph of \(S_X\) or area between the graph
of \(y=F_X(t)\) and the line \(y=1\). This still holds when \(X\) is discrete!
...
\end{cases}
\]
where \(x_{+}=\max\{x,0\}\) is the \defn{positive part} of \(x\).
...
\faIcon{building} insures the remaining portion (if exists). Then, the stop
...
``stops'' the loss suffered by \faIcon{user} from \(d\) dollars onwards, hence
``stop loss''.
\end{note}
...
Let \(X\) be a random variable with finite mean and let \(S_X(x)\) be its
survival function. Then, for any \(d\in\R\),
\[
...
\end{tikzpicture}
\begin{pf}
Since \((X-d)_{+}\ge 0\), by \Cref{prp:mean-surv} we have
\[
...
\item Alternatively, one can compute the expectation by definition. However,
\Cref{prp:mean-sl-surv} is often more handy.
...
needs to bear the full responsibility of it anyway.

Hence, such losses are \emph{not found} in \faIcon{building}'s record (database
\faIcon{database}, spreadsheet \faIcon{file-excel}, etc.), meaning that
they \emph{do not exist} from \faIcon{building}'s perspective. Thus, we have an
...
An important relationship between stop loss and limited loss variables is as
follows. For any \(d\in\R\) and any random variable \(X\),
\[
(X-d)_{+}+(X\wedge d)=X.
\]
\begin{pf}
...
\begin{remark}
\item A practical interpretation of this result is that combining an insurance with
deductible \(d\) and another insurance with policy limit \(d\) gives an
insurance with full coverage.
\item We can also show that \((d-X)_{+}+(d\wedge X)=d\) using a similar proof.
This provides a helpful way to find probabilistic quantities about the
``swapped'' stop-loss variable \((d-X)_{+}\).
\end{remark}

\item Due to the relationship in \labelcref{it:sl-limloss-relation}, we can
obtain the following formula for computing expected limited loss, using
\Cref{prp:mean-surv,prp:mean-sl-surv}, assuming \(X\) has finite mean and is
nonnegative:
\[
...
\]
That is, existence of \(k\)th moment implies existence of
...
\]
That is, non-existence of \(r\)th moment implies non-existence of
