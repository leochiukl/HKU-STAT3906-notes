...
\(p_k=\prob{N=k}\) for any \(k=0,1,\dotsc\). Then, the distribution of \(N\) is in
the \defn{\((a,b,0)\) class} if there exist constants \(a\) and \(b\) such that
\[
p_k=\qty(a+\frac{b}{k})p_{k-1},\quad\text{for any \(k=1,2,\dotsc\)}.
\]
\begin{note}
...
distribution in the \((a,b,0)\) class.
...
distributions in the \((a,b,0)\) class.
\end{theorem}
\begin{pf}
...
will show that they \emph{are} distributions in the \((a,b,0)\) class in the
following.

\item \label{it:pois-ab0}
For \(N\sim\pois{\lambda}\), for any \(k=1,2,\dotsc\),
\[
...
\[
k\cdot\frac{p_k}{p_{k-1}}=ka+b\quad\text{for any \(k=1,2,\dotsc\)},
\]
...
claims}}{\text{total no.\ of policies considered}}\) for any \(k=0,1,\dotsc\).
...
choice of \(p_0\) on distribution in the \((a,b,0)\) class, which gives rise to
the \emph{\((a,b,1)\) class}.

\item Let \(N\) be a nonnegative discrete random variable with pmf
\(p_k=\prob{N=k}\). Then, (the distribution of) \(N\) is in the \defn{\((a,b,1)\)
class} if there exist constants \(a\) and \(b\) such that
\[
p_k=\qty(a+\frac{b}{k})p_{k-1}\quad\text{for any \(k=\mgc{2},3,\dotsc\)},
\]
while \(p_0\) can take any value in \([0,1]\).
...
fixed after \(a\), \(b\), and \(p_0\) are fixed. Hence, the values of \(a\),
\(b\), and \(p_0\) can be used to uniquely characterize a distribution in the
\((a,b,1)\) class.
\end{enumerate}
\subsection{The Zero-Modified and Zero-Truncated \((a,b,1)\) Classes}
\begin{enumerate}
\item \label{it:zero-modify-process} One systematic method for creating a
distribution in the \((a,b,1)\) class is \defn{zero-modification}:
\begin{enumerate}[label={(\arabic*)}]
\item Pick any distribution of a nonnegative discrete random variable (usually
in the \((a,b,0)\) class).
\item Modify \(p_0\) to an arbitrary number \(p_0^{M}\in [0,1]\).
\item Modify the probabilities \(p_1\), \(p_2\), \(\dotsc\) to \(p_1^M=cp_1\),
\(p_2^M=cp_2\),\(\dotsc\) respectively, for some constant \(c\), such that the
resulting probabilities \(p_0^{M},p_1^{M},\dotsc\) form a valid distribution.
\end{enumerate}
Then, \(p_0^M,p_1^M,\dotsc\) form a distribution in the \defn{zero-modified
\((a,b,1)\) class}.

In the case where we start with a distribution in the \((a,b,0)\) class, for
any \(k=2,3,\dotsc\), we have
\[
p_k^{M}=cp_k=c\qty(a+\frac{b}{k})p_{k-1}
=\qty(a+\frac{b}{k})p_{k-1}^{M}.
\]
...
Let \(N\) be a random variable in the \((a,b,0)\) class, and let \(N^M\) be
the random variable in the zero-modified \((a,b,1)\) class obtained by applying
zero-modification on the distribution of \(N\). Then, for any \(k=1,2,\dotsc\), the
...
=\boxed{c\expv{N^k}}
\]
where \(\displaystyle c=\frac{1-p_0^M}{1-p_0}=\frac{1-\prob{N^M=0}}{1-\prob{N=0}}\).

\begin{warning}
We do \underline{not} have \(\vari{N^M}=c\vari{N}\)!
\end{warning}

\item \label{it:zm-pgf-fmlas}
Consider the same setting as \labelcref{it:zm-kth-moment}. The pgf of \(N^M\)
is
 \[
P_{N^M}(t)=p_0^M+tp_1^M+t^2p_2^M+\dotsb
...
\begin{intuition}
The final expression \(\boxed{\mgc{p_0^M+}\gc{c(}P_N(t)\vc{-p_0}\gc{)}}\) corresponds to the
``actions'' taken on the pgf during the zero modification:
\begin{enumerate}[label={(\arabic*)}]
\item \(\vc{-p_0}\): ``take away'' the original \(p_0\) from the initial pgf
\item \(\gc{c(\cdot)}\): scale \(p_1,p_2,\dotsc\) by \(c\)
\item \(\mgc{p_0^M+}\): ``add back'' the modified probability \(p_0^M\)
\end{enumerate}
\end{intuition}

We can also rewrite the expression as follows:
\[
p_0^M+c(P_N(t)-p_0)
...
\(p_k^M\) by \(p_k^T\) instead for any \(k=0,1,\dotsc\).
...
\begin{itemize}
\item \(\displaystyle c=\frac{1}{1-p_0}\), and \(p_k^T=cp_k\) for any \(k=1,2,\dotsc\).
...
This allows us to create a distribution in the \((a,b,1)\) class that
\emph{cannot} be obtained by zero-modification.
...
\]
for any \(k=0,1,\dotsc\). Recall that the constraints on the parameters are \(r>0\)
and \(\beta>0\).

Now, suppose we improperly set \(r\in(-1,0)\), while \(\beta>0\) still. A
consequence is that
\[
p_0=(1+\beta)^{-r}>1\qqtext{and}p_k<0\text{ for any \(k=1,2,\dotsc\)},
\]
...
\[
p_0^T=0\qqtext{and}p_k^T=cp_k\quad\text{for any \(k=1,2,\dotsc\)}
\]
...
\(k=0,1,\dotsc\), since \(p_k<0\) for any \(k=1,2,\dotsc\) and \(c<0\). Thus, the
...
\[
p_k=\binom{r+k-1}{k}\qty(\frac{1}{1+\beta})^{r}\qty(\frac{\beta}{1+\beta})^{k}
\]
for all \(k\) needed, depending on the method chosen in the second step below.
...
formula to compute the rest of the probabilities needed:
\[
p_k^T=\qty(a+\frac{b}{k})p_{k-1}^{T}\quad\text{for any \(k=2,3,\dotsc\)}
\]
where \(\displaystyle a=\frac{\beta}{1+\beta}\) and \(\displaystyle
